...
(function() {
	...
	function main(){
		if(document.querySelector('.header-scrobble-since')){
	...
	const callback = function(mutationsList, observer){
		for(const mutation of mutationsList){
			if(mutation.addedNodes.length){
				if(document.URL.indexOf('https://www.last.fm/user/') !== -1){
					if(document.URL.split('https://www.last.fm/user/')[1].indexOf('/') === -1){
						main();
					}
				}
			}
